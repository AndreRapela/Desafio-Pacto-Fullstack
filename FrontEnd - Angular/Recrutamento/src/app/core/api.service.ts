...
export class ApiService {
  ...
  updateJob(id: string, payload: JobPayload): Observable<Job> {
    return this.http.put<Job>(`/api/jobs/${id}`, payload);
  }
  deleteJob(id: string): Observable<void> {
    return this.http.delete<void>(`/api/jobs/${id}`);
  }
  apply(jobId: string): Observable<CandidateApplication> {
    return this.http.post<CandidateApplication>(`/api/applications/jobs/${jobId}`, {});
  }
  myApplications(): Observable<CandidateApplication[]> {
    return this.http.get<CandidateApplication[]>('/api/applications/me');
  }
  ...
  updateApplication(
    ...
    return this.http.patch<CandidateApplication>(`/api/applications/${id}/status`, {
      status,
      feedback,
    });
  }
  ...
  markNotificationRead(id: string): Observable<NotificationItem> {
    return this.http.patch<NotificationItem>(`/api/notifications/${id}/read`, {});
  }

  notification(id: string): Observable<NotificationItem> {
    return this.http.get<NotificationItem>(`/api/notifications/${id}`);
  }
}
